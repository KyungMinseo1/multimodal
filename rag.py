...
class RAGPipeline:
    def __init__(self, vector_db_path: str = "static/경영정보시스템5_chroma_db"):
...
"""
    ...
    def search_documents(self,
                        ...
                        page_range: Optional[Tuple[int, int]] = None) -> List:
        """고급 검색 기능"""

        # 기본 유사도 검색
        if pdf_filter or page_range:
            # 메타데이터 필터링이 필요한 경우
            all_docs = self.vector_store.similarity_search(query, k=k*3)  # 더 많이 가져와서 필터링
   ...
   "정보 처리 주기에 대해 설명해주세요"
    ...
    def _handle_command(self, command: str):
        ...
        if command.startswith('/search '):
            ...
            for i, doc in enumerate(docs, 1):
                metadata = doc.metadata
                print(f"   [{i}] {metadata.get('pdf_filename')} (p.{metadata.get('page_number')})")
