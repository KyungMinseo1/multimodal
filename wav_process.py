...
class FastAudioPreprocessor:
    def __init__(self, save_dir, scaler, sr=48000, frame_size=512, hop_size=256, device='cuda'):
    ...
    def preprocess_and_save(self, wav_file, save_name):
        """n_jobs=1로 GPU 메모리 충돌 방지"""
        print("🎵 Starting audio processing...")
        # GPU 사용시 순차 처리, CPU 사용시 병렬 처리
        try:
            features_tensor = self._process_single_file(wav_file, save_name)
            if features_tensor is not None:
                save_filename = save_name
                save_path = os.path.join(self.save_dir, save_filename)
                torch.save(features_tensor.cpu(), save_path)
        except Exception as e:
            print(f"Error processing {save_name}: {e}")
        return save_path

    def _process_single_file(self, wav_file, save_name):
        try:            
            waveform, orig_sr = torchaudio.load(wav_file)
            
            if orig_sr != self.sr:
                resampler = torchaudio.transforms.Resample(orig_sr, self.sr).to(self.device)
            ...
            if self.scaler is not None:
                ...
                features = torch.tensor(features_np, device=self.device, dtype=torch.float32)
    ...
    def frame_signal(self, signal):
        ...
        frames = signal.unfold(0, self.frame_size, self.hop_size)  # [num_frames, frame_size]
    ...
    def compute_spectral_features(self, frame):
        ...
        rolloff = freqs[rolloff_idx[0]] if len(rolloff_idx) > 0 else torch.tensor(0.0, device=self.device)
        
        # Spectral bandwidth
        bandwidth = torch.sqrt(torch.sum(((freqs - centroid) ** 2) * spectrum) / (torch.sum(spectrum) + 1e-10))
    ...
    def extract_features(self, frames):
        ...
        # Bandwidth  
        bandwidth = torch.sqrt(torch.sum(((freqs - centroid.unsqueeze(1)) ** 2) * spectrum_abs, dim=1) / total_energy.squeeze(1))
        ...
        for i in range(B):
            ...
            mfcc_single = self.mfcc_transform(frame_for_mfcc)  # [1, n_mfcc, time_frames]
        ...
        # 5. ZCR [B, 1]
        zcr = torch.mean((frames[:, 1:] * frames[:, :-1] < 0).float(), dim=1, keepdim=True)
...
def load_preprocessor(save_dir):
    load_path = 'model/train_dataset_scaler_gpu.pkl'
    scaler = joblib.load(load_path)
    preprocessor = FastAudioPreprocessor(save_dir, scaler=scaler)
    return preprocessor

# 사용법 함수
def preprocess_audio_dataset(preprocessor, wav_file, save_name):
    """오디오 데이터셋 전처리 실행"""
    save_path = preprocessor.preprocess_and_save(wav_file, save_name)
    return save_path