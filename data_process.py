...
def analyze_concentration_changes(data: List[Dict]) -> Dict[str, Any]:
    ...
    # 집중도 매핑 (숫자가 낮을수록 집중도 높음)
    # 4: 졸음, 3: 지루함(집중하지 않음), 2: 차분함(집중하지 않음), 1: 차분함(집중함), 0: 흥미로움(집중함)
    ...
    for entry in data:
        ...
        # 시선 데이터
        total_yaw.append(abs(entry['pose']['yaw']))  # 절댓값으로 변환 (좌우 움직임 정도)
        total_pitch.append(abs(entry['pose']['pitch']))  # 절댓값으로 변환 (상하 움직임 정도)
    ...
    for i, entry in enumerate(data[1:], 1):  # 두 번째 항목부터 시작
        ...
        concentration_state_change = is_concentration_state_changed(previous_score, current_score)
        ...
        if score_change >= 2 or concentration_state_change:
            ...
            if i > current_segment_start:
                ...
                change_points.append({
                    ...
                    'duration_seconds': segment_data[-1]['timestamp']['end'] - segment_data[0]['timestamp']['start'],
                    'avg_concentration_score': round(statistics.mean(segment_concentration), 2),
                    'concentration_state': get_concentration_state(statistics.mean(segment_concentration)),
                    ...
                    'head_stability': get_head_stability(statistics.mean(segment_yaw), statistics.mean(segment_pitch)),
    ...
    result = {
        'session_overview': {
            'total_duration_seconds': data[-1]['timestamp']['end'] - data[0]['timestamp']['start'],
            'total_duration_minutes': round((data[-1]['timestamp']['end'] - data[0]['timestamp']['start']) / 60, 1),
        ...
        'learning_insights': generate_insights(change_points, avg_concentration, dominant_noise)
    }
...
def get_dominant_noise_in_segment(segment_data: List[Dict]) -> str:
    """구간 내 주요 소음 타입 찾기"""
    noise_labels = {0: "없음", 1: "무음", 2: "자동차", 3: "사이렌", 4: "키보드", 5: "대화소리"}
    noise_counts = {}
...
def generate_insights(segments: List[Dict], avg_concentration: float, dominant_noise: str) -> List[str]:
    """학습 패턴 인사이트 생성"""
    insights = []
    ...
    if avg_concentration <= 1:
        ...
        insights.append("집중도가 전반적으로 낮았습니다. 학습 환경이나 방법을 점검해보세요.")
    ...
    if len(segments) >= 2:
        ...
        first_avg = statistics.mean([seg['avg_concentration_score'] for seg in first_half])
        second_avg = statistics.mean([seg['avg_concentration_score'] for seg in second_half])
        
        if first_avg < second_avg:  # 점수가 낮을수록 집중도 높음
            insights.append("학습 초반에 더 집중했습니다. 시간이 지날수록 집중도가 떨어지는 경향이 있어요.")
        elif first_avg > second_avg:
            insights.append("시간이 지날수록 집중도가 향상되었습니다! 좋은 패턴이에요.")
    
    # 시선 안정성 분석
    unstable_segments = [seg for seg in segments if seg['head_stability'] in ['불안정', '보통']]
    if len(unstable_segments) > len(segments) / 2:
        insights.append("시선이 자주 움직였습니다. 한 곳에 집중하는 연습이 필요해 보여요.")
    
    # 소음 영향 분석
    if dominant_noise == "사이렌":
        insights.append("사이렌 소리에 특히 민감하게 반응하는 것 같습니다.")
    elif dominant_noise == "대화소리":
        insights.append("대화 소리가 집중에 방해가 되는 것 같습니다. 조용한 환경에서 학습해보세요.")
    ...
    if len(focused_segments) > 0:
        avg_focused_duration = statistics.mean([seg['duration_seconds'] for seg in focused_segments])
...
if __name__ == "__main__":
    ...
    # 결과 출력 (예쁘게 포맷팅)
    print("=== 학습 세션 분석 결과 ===")
    print(f"총 학습 시간: {analysis_result['session_overview']['total_duration_minutes']}분")
    print(f"평균 집중도: {analysis_result['session_overview']['avg_concentration_state']}")
