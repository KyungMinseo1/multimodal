...
def plot_feedback_and_df(result, user_name) -> tuple:
    """학습 몰입도 분석 결과를 시각화하는 함수"""
    ...
    # 📈 그래프 그리기
    plt.plot(x, y, linestyle='-', color='#4CAF50', linewidth=2, marker='o', markersize=5, markerfacecolor='white')
    ...
    plt.tight_layout()
    plt.savefig(f'static/feedback_{user_name}.png', dpi=300)
    ...
    return data_str, stt_chunk

if __name__=="__main__":
    result_path = f'result_경민서_modified.json'
    data = os.path.join('analysis', result_path)
    with open(data, 'r', encoding='utf-8') as f:
        result = json.load(f)
    result = sorted(result, key=lambda x: x['timestamp']['start'])
    data_str, stt_chunk = plot_feedback_and_df(result, "경민서")
