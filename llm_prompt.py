...
"""
    ...
    def _rag_process(self, topic:str, stt_chunk:list) -> str:
        ...
        """
        result = []
        for chunk in stt_chunk:
            rag_output = self.rag.ask_question(chunk[0], topic)
            topic_output = rag_output['answer']
            relevant_docs_output = rag_output['relevant_docs']
            if topic_output[0][0] == "":
                continue
            result.append([topic_output, chunk[1]])
            self.relevant_docs.extend(relevant_docs_output)
        return str(result)
