...
class TransformerFusion(nn.Module):
    ...
    def forward(self, visual_embed, audio_embed):
        ...
        # 전체 시퀀스 생성: (B, 3, 128)
        fusion_input = torch.cat([cls_tokens, visual_embed, audio_embed], dim=1)
...
class EndtoEndModel(nn.Module):
    def __init__(self, mobile_net, eye_track_model, noise_classifier,
                 face_mesh_model, visual_proj, audio_proj,
    ...
    def forward(self, cropped_faces, audio_features):
        ...
        visual_feature = torch.cat([img_features, face_points_batch], dim=1)  # [B, 1024 + 136]
        visual_proj_input = torch.cat([visual_feature, eye_features], dim=1)  # [B, 1024+136+2048=~4208]
        z_visual = self.visual_proj(visual_proj_input)
...
def load_model():
    ...
    # Assume visual_proj and audio_proj output 128-dim each
    visual_proj = ProjectionHead(input_dim=3464, proj_dim=128)
    audio_proj = ProjectionHead(input_dim=256, proj_dim=128)

    fusion_block = TransformerFusion(embed_dim=128, num_heads=4, num_layers=2)
    final_classifier = FinalClassifier(embed_dim=128, num_classes=5)
    ...
    results = torch.load('checkpoints_transformer_fusion/best_model.pth')
    e2e_model.load_state_dict(results['model_state_dict'])
    face_box.eval()
    e2e_model.eval()
...
def run(face_box, e2e_model, img, aud):
    ...
    def preprocess_batch_tensorized(face_box, img):
        ...
        def resize_tensor_image(img_tensor, size=(640, 640)):
            return F.interpolate(img_tensor.unsqueeze(0), size=size, mode='bilinear', align_corners=False).squeeze(0)
        
        def resize_cropped_face(face, size=(448, 448)):
            return F.interpolate(face.unsqueeze(0), size=size, mode='bilinear', align_corners=False).squeeze(0)
            ...
            try:
                ...
                try:
                    if len(result.boxes) > 0:
                        ...
                        x2, y2 = min(img_tensor.shape[-1], x2), min(img_tensor.shape[-2], y2)
    ...
    with torch.no_grad():
        ...
        label_dict = {0: '집중_흥미로움', 1: '집중_차분함', 2: '집중하지않음_차분함', 3: '집중하지않음_지루함', 4: '졸음_알수없음'}
        noise_label_dict = {0: '없음', 1: '무음', 2: '자동차', 3: '싸이렌', 4: '키보드', 5: '대화소리'}
        result = label_dict.get(int(predicted), "Unknown")
        noise_result = noise_label_dict.get(int(noise_predicted), "Unknown")
        return (int(predicted), result), (yaw, pitch), (int(noise_predicted), noise_result)