...
def load_model():
    ...
    state_dict = torch.load('model/l2cs_trained.pkl', map_location=torch.device('cuda'))
...
def predict_batch(yaw_logits, pitch_logits, device=None):
    ...
    idx_tensor = torch.arange(NUM_BINS, dtype=torch.float32, device=device).unsqueeze(0)  # [1, NUM_BINS]
...
def predict(yaw_logits, pitch_logits, device=None):
    ...
    pitch_probs = F.softmax(pitch_logits, dim=1)
    idx_tensor = torch.arange(NUM_BINS, dtype=torch.float32, device=yaw_logits.device).unsqueeze(0)
